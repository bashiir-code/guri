...
function MyRequests() {
  ...
  // Past sections start collapsed so the screen stays about what's in play.
  const [openSection, setOpenSection] = useState<'closed' | 'ended' | null>(null);
  ...
  // Active up top; the finished ones grouped by how they finished — a rented
  // home (closed) is a different memory than a withdrawn or declined request.
  const active = requests?.filter((r) => OPEN.includes(r.state)) ?? [];
  const closed = requests?.filter((r) => r.state === 'closed') ?? [];
  const ended = requests?.filter((r) => !OPEN.includes(r.state) && r.state !== 'closed') ?? [];

  // Auto-expand the first ACTIVE request so the timeline is visible on arrival.
  const firstId = active[0]?.id;
  const openId = expanded ?? firstId ?? null;

  const renderCard = (r: RequestRow) => {
    ...
    return (
      <div key={r.id} className="animate-rise-in overflow-hidden rounded-card border bg-card">
        <button
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <span
              className={cn(
                'mt-2 inline-block rounded-full px-2.5 py-1 text-[11.5px] font-semibold',
                OPEN.includes(r.state) || r.state === 'closed'
                  ? 'bg-lime/[0.35] text-forest'
                  : 'bg-muted text-slate_brand',
              )}
            >
              {inPipeline && OPEN.includes(r.state)
                ...
                return (
                  <li key={step} className="flex gap-4">
                    <div className="flex w-5 flex-none flex-col items-center">
                      ...
                      >
                        {done && (
                          <Check className="h-2.5 w-2.5 text-lime" strokeWidth={3.5} aria-hidden />
                        )}
  ...
  const sectionToggle = (section: 'closed' | 'ended', label: string, count: number) => (
    <button
      onClick={() => setOpenSection(openSection === section ? null : section)}
      className="flex w-full items-center justify-between rounded-full border px-4 py-2.5 text-left"
      aria-expanded={openSection === section}
    >
      <span className="text-[13px] font-semibold text-slate_brand">
        {label}
        <span className="ml-2 rounded-full bg-muted px-2 py-0.5 text-[11.5px] font-bold text-forest">
          {count}
        </span>
      </span>
      <ChevronDown
  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-[560px] px-4 py-4 lg:max-w-2xl lg:px-6">
      ...
      {active.length > 0 && <div className="flex flex-col gap-4">{active.map(renderCard)}</div>}
      {signedIn && !isLoading && active.length === 0 && (closed.length > 0 || ended.length > 0) && (
        <p className="py-2 text-center text-sm text-muted-foreground">{t('noActive')}</p>
      )}

      {closed.length > 0 && (
        <section className="mt-6">
          {sectionToggle('closed', t('sectionClosed'), closed.length)}
          {openSection === 'closed' && (
            <div className="mt-3 flex flex-col gap-4">{closed.map(renderCard)}</div>
          )}
        </section>
      )}
      ...
      {ended.length > 0 && (
        <section className="mt-4">
          {sectionToggle('ended', t('sectionEnded'), ended.length)}
          {openSection === 'ended' && (
            <div className="mt-3 flex flex-col gap-4">{ended.map(renderCard)}</div>
          )}
        </section>
      )}
